...
filename = _thisDir + os.sep + u'data/%s_%s_%s' % (expInfo['participant'], exp_name, expInfo['date'])
...
def create_image(stim):
    ...
    img.save(filename + '%s' % (variables.picture_number) + ".jpg", "JPEG", quality=80, optimize=True, progressive=True)
...
#                    trials_loop.addData('signal_picture', save_pic)
                    ...
                    trials_loop.addData('picture_name', '%s_%s_%s' % (expInfo['participant'], exp_name, expInfo['date']) + '%s' % (variables.picture_number))
