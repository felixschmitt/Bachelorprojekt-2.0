...
filename = _thisDir + os.sep + u'data/%s/%s_%s' % (expInfo['participant'], exp_name, expInfo['date'])
...
thisExp = data.ExperimentHandler(name=exp_name, version='',
    ...
    originPath='C:\\Users\\Leon\\Documents\\GitHub\\Bachelorprojekt-2.0\\SIGNALENTDECKUNGSTHEORIE',
...
conditions=[{'label':'Signal Stärke'}]
...
#trials = data.TrialHandler(nReps= (variables.trial_blocks * variables.number_of_trials),
#    method='sequential', 
#    extraInfo=expInfo, 
#    originPath= None,
...
info['nTrials']= variables.trial_blocks
info['observer']='jwp'
...
for thisStart in info['startPoints']:
    ...
    thisStair = data.StairHandler(startVal=thisStart,
        nReversals = variables.number_of_trials,
        extraInfo=thisInfo,
        nTrials=variables.number_of_trials, nUp=0, nDown=0.5,
        minVal = 0.5, maxVal=8, 
        stepSizes=[1,1,1,1,1,1])
 ...
 ################################## 
        ...
        if trial == max_trial:
            ...
            if testtrial_condition:
                ...
                max_trial = variables.number_of_trials
                testtrial_condition = False
                countdown_clock = core.CountdownTimer(1)
...
#        while trial < max_trial and trial_blocks < variables.trial_blocks:
        ...
        #            for thisTrial in trials:
                    ...
                    thisStair.addData('signal_intensity', image_factory.variables.signal_intensity)
#                    trials_loop.addData('signal_picture', save_pic)
                    thisStair.addData('response', response)    
                    print(thisStair.data[0])
                    print(thisStair.data[1])
...
for thisStair in stairs:
    ...
    filename_stair = _thisDir + os.sep + u'data/%s/%s/data' % (expInfo['participant'],thisStair.extraInfo['thisStart'])
    thisStair.saveAsPickle(filename_stair)
    thisStair.saveAsText(filename_stair)   


#thisStair.saveAsExcel(fileName= filename_trial,
#                  sheetName = 'rawData',
#                  matrixOnly=True)
